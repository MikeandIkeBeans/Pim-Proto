...
from datetime import datetime, timezone  # ⬅️ UPDATED: include timezone
...
class MediaPipePoseProcessor:
    ...
    def __init__(self, process_every_n_frames: int = 8, enable_classifier: bool = True,
        ...
        LOGGER.info("📊 MediaPipe Pose initialized (model_complexity=1, workers=%d)", max_workers)
    ...
    def _collect_pending_result(self):
        ...
        if self._pending_future is not None and self._pending_future.done():
            try:
                ...
                LOGGER.error("❌ Error collecting AI result: %s", e)
            finally:
...
async def publish(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            LOGGER.info("🦴 MediaPipe skeleton overlay enabled (background threaded)")
