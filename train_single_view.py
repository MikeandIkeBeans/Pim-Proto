...
- ✅ Loads multi-view NPZ files but randomly selects ONE view per sample for TRAIN
- ✅ Validation can use a FIXED view (deployment-like) via --val_fixed_view_idx
- ✅ Validation uses a separate stride (defaults to non-overlapping) to reduce correlation
...
def analyze_window_purity(dataset: NPZWindowDataset) -> Dict:
    ...
    if not any(fl is not None for fl in getattr(dataset, 'frame_labels_per_file', [])): print("⚠️  No frame-level labels."); return {}
...
def filter_windows_by_purity(dataset: NPZWindowDataset, indices: List[int], min_purity: float = 0.8) -> List[int]:
    if dataset.lazy_load or not dataset.use_soft_labels:
        print("⚠️  Cannot filter by purity (lazy or no soft labels)."); return indices
    if not any(fl is not None for fl in getattr(dataset, 'frame_labels_per_file', [])):
        print("⚠️  No frame-level labels for purity filtering."); return indices
    kept = []
    for idx in indices:
    ...
    print(f"\n🎯 Window Purity Filtering: kept {len(kept)}/{len(indices)}")
    return kept
...
def _rand_bbox_2d(T: int, F: int, lam: float, temporal_only: bool = False):
    """
    Sample a (time x feature) rectangle with area ≈ (1 - lam) * (T*F).
...
def main():
    ...
    print(f"✓ Single-view input_dim: {train_ds.input_dim} (33 landmarks × 3)")
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        if 'classes' in ckpt and ckpt['classes'] != classes:
            raise ValueError("Checkpoint classes ≠ current classes. Refusing to resume to avoid label mismatch.")
    ...
    print(f"🎯 SINGLE-VIEW MODE: Random view selection per sample (train).")
    ...
    if args.cutmix_alpha > 0: print(f"CutMix: α={args.cutmix_alpha}, prob={args.cutmix_prob}, temporal_only={args.cutmix_temporal_only}")
    ...
    if val_stride_s is not None:
        ...
        print(f"Validation stride (frames): {val_default_stride} (T={args.T} → {'non-overlapping' if val_default_stride==args.T else 'overlapping'})")
